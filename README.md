...
- ⌨️ [Typed.js](https://github.com/mattboldt/typed.js/) — typewriter effect
...
│
├── js/
│   ├── script.js           # ⚙️  Main interactivity (toast, form, skills toggle)
│   ├── particles.js        # ✨ tsParticles configuration
│   ├── typed.js            # ⌨️  Typewriter effect for the About section
...
│
├── assets/
│   └── img/                # 🖼️  Project images and pet photos
│
├── .gitignore
...
- **Contact 📬** — A working contact form with name, email, and message fields. On submission, the button changes text and color to confirm the action, then resets automatically.
...
- **Gallery — Grid mosaic 🗃️** — The same three pets shown again in a CSS Grid layout with a wide card spanning multiple columns, demonstrating a different layout strategy with the same components.
...
- ⌨️ Typewriter effect triggered only when the About section enters the viewport.
- 🃏 Project cards with CSS flip animation revealing details on hover
- ❤️ Like buttons on pet cards with icon toggle and bounce animation
...
> ⚠️ Some features work best when served from a local server. See Option 2.
...
The page will open in your browser with automatic reloading on every file change. 🔄

---

## 📦 External Libraries (CDN)

All external dependencies load via CDN — no `npm install` or build step required. 🎉
